...
What is faulty about this algorithm? We first stated that it would be no good to count directly on $B$ for we would then have to traverse it in $O(n^2)$ time. The issue is in verifying if we have already seen an element $a$ when looking up the index to its counter: in order to be able to distinguish a new element from one previously seen we must zero out $B$. There is no way around it; this takes $O(n^2)$ time, so we are back where we started.

\medskip

Therefore it seems our best bet is once again to build a binary search tree that holds a count along with each key. The tree will have at most $\log\log n$ elements, and therefore $\log\log\log n$ levels, so building it and traversing it will be $O(n\log\log\log n)$ time---not too bad, but still more than the $O(n)$ time it will take to output all the original $n$ elements.
