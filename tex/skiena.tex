...
\paragraph{2-14} $\Theta\!\left(n^2\right) = \Theta\!\left(n^2+1\right)$? This is to say whether both classes are the same. We can show this by assuming we have $f(n) = \Theta\!\left(n^2\right)$, and proving that $f(n) = \Theta\!\left(n^2+1\right)$, and likewise with the other assumption.

First, $f(n) = \Theta\!\left(n^2\right) \Rightarrow f(n) = O\!\left(n^2\right)$ and $f(n) = \Omega\!\left(n^2\right)$. Is $f(n) = O\!\left(n^2+1\right)$? This would mean $f(n) \le c\left(n^2+1\right)$, for $n > n_0$. Since $f(n) = O\!\left(n^2\right)$ there is $c_0$ such that $f(n) \le c_0 n^2$, but $c_0 n^2 \le c_0\left(n^2 + 1\right)$ so with the same $c_0$ we see that $f(n) = O\!\left(n^2+1\right)$.

Now we want to show that $f(n) = \Omega\!\left(n^2+1\right)$, that is, $c\left(n^2 + 1\right) \le f(n)$ for $n > n_0$. Since $f(n) = \Omega\!\left(n^2\right)$ there are $c_1, n_1$ such that $c_1 n^2 \le f(n)$, for $n > n_1$. In particular this holds for $n+1 > n_1$, so
\begin{equation*}
	c_1 n^2 < c_1 (n+1)^2 \le f(n), \quad n > n_1.
\end{equation*}
But note that $n^2 + 1 \le (n+1)^2$, so for $n_0 = n_1 + 1$ above, we have that $c_1\left(n^2 + 1\right) \le c_1 (n+1)^2 \le f(n)$ for $n > n_0$. So $f(n) = \Omega\!\left(n^2+1\right)$ and thus $f(n) = \Theta\!\left(n^2 + 1\right)$.

\vskip 1em

Now we need to show that $f(n) = \Theta\!\left(n^2+1\right) \Rightarrow f(n) = \Theta\!\left(n^2\right)$.

$f(n) = \Omega\!\left(n^2\right) \Leftrightarrow c n^2 \le f(n)$ for $n > n_0$. We know that $f(n) = \Omega\!\left(n^2+1\right)$ so there is a $c_1$ such that $c_1\left(n^2+1\right) \le f(n)$ for $n > n_1$; since $n^2 < n^2 + 1$, by letting $c = c_1$ we see that $c_1 n^2 < c_1 \left(n^2+1\right) \le f(n)$, so $f(n) = \Omega\!\left(n^2\right)$.

$f(n) = O\!\left(n^2\right) \Leftrightarrow f(n) \le c n^2$ for $n > n_0$. Since $f(n) = O\!\left(n^2 + 1\right)$ we know there are $c_1$ and $n_1$ such that $f(n) \le c_1\left(n^2+1\right)$ for $n \ge n_1$; in particular, $f(n) \le c_1\left(n_1^2+1\right)$. Then let $c = c_1\left(n_1^2+1\right)$, then $c n^2 > c_1\left(n_1^2+1\right) \ge f(n)$, for $n > n_1$. Thus $f(n) = O\!\left(n^2\right)$ and $f(n) = \Theta\!\left(n^2\right)$.

\vskip 1em

This shows that if a function is $\Theta\!\left(n^2\right)$ then it must be $\Theta\!\left(n^2+1\right)$ and viceversa---that is, both classes are the same.
...
\paragraph{2-21} We are to prove that $p(n) = a_k n^k + \ldots + a_0 = O\!\left(n^k\right)$, for $k \ge 0$ and arbitrary real coefficients $a_i$. This is to say that we can find a $c > 0$ and $n_0$ such that $p(n) \le c n^k$, for all $n > n_0$.
...
Now take $c = \max(a_0,\ldots, a_k)$. Then clearly $c n^k + \ldots + c \ge a_k n^k + \ldots + a_0 = p(n)$. But because of the same argument as above, there is some $n_0$ after which $c n^k > c n^{k-1} + \ldots + c$. Then it follows that $p(n) \le c n^k$ for $n > n_0$, which means $p(n) = O\!\left(n^k\right)$.
\end{proof}

\paragraph{2-22} Let $a, b \in \R$, with $b > 0$. Show that $(n+a)^b = \Theta\!\left(n^b\right)$.

\begin{proof}
To prove that $(n+a)^b = O\!\left(n^b\right)$ we need to find $c_0$ such that $c_0 n^b \ge (n+a)^b$ for sufficiently large $n$. If $a=0,\ (n+a)^b = n^b$ and with $c=1$ the inequality holds. If $a<0, (n+a)^b \le n^b$, so
\begin{equation*}
...
Note that $\frac{a}{n}$ can get arbitrarily close to 0 for large enough $n$, so the entire expression tends to 1 as $n\to\infty$. Thus for, say, $c=2$ it is possible to find $n_0$ large enough such that $(n+a)^b \le 2n^b$. So $(n+a)^b = O\!\left(n^b\right)$.

\vskip 0.5em

To prove that $(n+a)^b = \Omega\!\left(n^b\right)$, by the same argument, $c n^b \le (n+a)^b \rightarrow c \le (n+a/n)^b = (1 + a/n)^b$. For $a\ne0$ the expression $1+a/n$ tends to 1, either ``from below'' or ``from above''---at any rate, by taking some $c<1$, say, $1/2$, it will be possible to find $n$ large enough that the inequality will hold. Thus $(n+a)^b = \Omega\!\left(n^b\right)$, and finally $(n+a)^b = \Theta\!\left(n^b\right)$.
\end{proof}
...
\paragraph{2-30}
\begin{enumerate}[label=(\alph*)]
	\item Yes---$O\!\left(n^2\right)$ worst case time doesn't necessarily mean it ever takes $n^2$ steps on any input. If the algorithm was $O(n)$ worst case, it would still be $O\!\left(n^2\right)$.
	\item Yes---$O\!\left(n^2\right)$ only talks about an upper bound.
	\item Yes. It could be that the best case time complexity of the algorithm is $O(n)$.
	\item No. Some inputs will trigger worst case behavior so will necessarily be $\Theta\!\left(n^2\right)$.
